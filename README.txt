...
Les fichiers sont répartis de la manière suivante:
  - les fichiers réponse, journal et conception (dont la lecture est conseillée) se trouvent dans le dossier INFOS
  ...
  - les fichiers tests (dont certains ne sont plus compilable) se trouvent dans le dossier TESTS
  - le code lié à l'affichage format texte se trouve dans le dossier vue_texte
  - le code qui a permis à la création du graphique concernant l'influence de la forme de l'accélérateur sur la stabilité des particules (cf. fichier conception) se trouve dans TESTS/testPolygone
...
Note : Pour obtenir un fichier exécutable (et pas un fichier ouvrable dans le terminal), il faut enlever la ligne "CONFIG -= app_bundle" dans les .pro correspondant.
...
Si vous désirez afficher la simulation sous format texte, il faudra suivre les deux premières étapes indiquées ci-dessus, puis ensuite écrire "./TESTS/exerciceP10/exerciceP10". Ceci lancera la simulation texte. Cependant, pour modifier les caractéristiques du faisceau et de l'accélérateur, il faudra les modifier directement sur le code source (cf. fichier conception pour avoir plus d'informations sur les méthodes à utiliser et les constructeurs à appeller).

3. Remarques concernant l'utilisation du programme (et conseils)

Pour s'amuser un peu avec le programme, il est conseillé de faire varier le nombre de côtés de l'accélérateur, et de faire varier les autres paramètres de distribution des faisceaux (pour voir leurs effets sur la forme des ellipses par exemple). Il est possible de passer des heures à tester des géométries et paramètres divers et variés, sans s'embêter (on parle par expérience) !
